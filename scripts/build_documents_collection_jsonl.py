#!/usr/bin/env python3
"""
Строит documents_collection.jsonl: для каждого вопроса — только релевантные документы
(doc_id + полный текст). Релевантный документ = есть хотя бы один чанк, чей текст
после normalize_for_dedup входит в long_answer (как в build_chunk_relevance_jsonl.py).

doc_id совпадает с build_chunks_collection_jsonl.py (хэш от URL, см. corpus_ids.py).

Запуск:
  python scripts/build_documents_collection_jsonl.py \\
    -i simpleqa_verified/simpleqa_verified_with_long_answer.csv \\
    -o simpleqa_verified/documents_collection.jsonl
"""
...
from corpus_ids import make_doc_id  # noqa: E402
from extract_long_answer import normalize_for_dedup, parse_documents  # noqa: E402
...
def doc_any_chunk_relevant(cells: list[dict], rel: set[str]) -> bool:
    for cell in cells:
        text = cell.get("text")
        if text is None:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CSV → documents_collection.jsonl")
    parser.add_argument(
        "-i",
        "--input",
        type=Path,
        default=ROOT / "simpleqa_verified" / "simpleqa_verified_with_long_answer.csv",
        help="CSV с колонками urls, chunks, long_answer, documents",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=ROOT / "simpleqa_verified" / "documents_collection.jsonl",
    ...
    required = ("urls", "chunks", "long_answer", "documents")
    lines_out = 0
    total_rel_docs = 0
    ...
    with open(inp, encoding="utf-8", newline="") as fin, open(out, "w", encoding="utf-8") as fout:
        ...
        for row_idx, row in enumerate(reader, start=1):
            ...
            for j in range(n_docs):
                ...
                if not doc_any_chunk_relevant(cells, rel):
                    continue
                ...
                docs_out.append({"doc_id": doc_id, "text": text})
                total_rel_docs += 1
    ...
    print(
        f"Вопросов: {lines_out}, релевантных документов всего: {total_rel_docs} → {out}"
    )
